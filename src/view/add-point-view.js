...
const createAddPointTemplate = (point, allOffers, allDestinations) => {
  const { type, basePrice, dateFrom, dateTo, destination, isDisabled, isSaving, } = point;
  ...
  const foundDestination = destination !== null ? getDestination(destination, allDestinations) : {};
...
export default class AddPointView extends AbstractStatefulView {
  ...
  #deleteClickHandler = (evt) => {
    evt.preventDefault();
    this._callback.deleteClick(AddPointView.parseStateToPoint(this._state));
  };
